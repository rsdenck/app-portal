...
function atendente_format_rate(?float $bps): string
{
    if ($bps === null) {
        return 'N/A';
    }
...
function atendente_format_duration(?float $seconds): string
{
...
if (!$host) {
    ...
    exit('Host não encontrado na API do Zabbix');
}
...
$roleLabel = ($user['role'] ?? '') === 'cliente' ? 'Cliente' : 'Atendente';
$backUrl = ($user['role'] ?? '') === 'cliente' ? '/cliente_hosts.php' : '/atendente_monitoramento.php';

$ticketUrl = '';
if (($user['role'] ?? '') === 'cliente') {
    $detailName = (string)($host['name'] ?? ($host['host'] ?? ''));
    $detailHost = (string)($host['host'] ?? '');
    $detailHostId = (string)($host['hostid'] ?? '');
    ...
    if ($detailHost !== '') {
        $query['extra']['hostname'] = $detailHost;
    }
    $ticketUrl = '/cliente_abrir_ticket.php?' . http_build_query($query, '', '&', PHP_QUERY_RFC3986);
}

render_header($roleLabel . ' · Host ' . (string)$host['host'], $user);
?>
<style>
.dash-card-grid {
    display: grid;
    ...
    <div class="resource-card">
        ...
        <div style="display: flex; gap: 16px; margin-top: 4px;">
            <div>
                ...
                <div style="font-weight: 700; color: var(--primary);"><?= h(atendente_format_rate($netIn)) ?></div>
            </div>
            <div>
                <div class="info-item-label">Upload</div>
                <div style="font-weight: 700; color: var(--primary);"><?= h(atendente_format_rate($netOut)) ?></div>
            </div>
        </div>
        <div class="resource-chart-container">
...
<div class="card">
    ...
    <div class="info-grid">
        <div class="info-item">
            ...
            <div class="info-item-value"><?= h(atendente_format_duration($uptimeSeconds)) ?></div>
        </div>
        <div class="info-item">
