...
// For client, get company_id from profile
$stmt = $pdo->prepare('SELECT cp.company_id, c.name as company_name 
...
$company_name = (string)($profile['company_name'] ?? 'Minha Empresa');
...
if ($company_id) {
    try {
        ...
        $stmt = $pdo->prepare('SELECT hostgroupid, name FROM zabbix_hostgroups WHERE client_user_id = ?');
        $stmt->execute([(int)$user['id']]);
        $hgs = $stmt->fetchAll();
        $gIds = [];
        foreach ($hgs as $hg) {
...
if ($generate) {
    try {
        ...
        $hosts = [];
        if ($tab !== 'sla') {
            // 1. Get hostgroups for the client's company (all users in company)
            ...
            if (empty($groupIds)) {
                // If strictly technical tab, throw error. For dashboard, we might just skip.
                if ($tab === 'technical') {
                    throw new RuntimeException('Nenhum grupo de hosts encontrado no Zabbix para sua empresa.');
                }
            } else {
        ...
        ");
        $stmt->execute([$company_id, date('Y-m-d H:i:s', time() - ($period * 86400))]);
        $tickets = $stmt->fetchAll();
        foreach ($tickets as $tk) {
            $ticketData[] = ticket_calculate_metrics($tk, $pdo);
        }

        // Initialize SLA variables to avoid undefined index warnings
        $slaMet = 0;
        $slaTotal = count($ticketData);
        foreach ($ticketData as $td) {
            if (isset($td['is_within_sla']) && $td['is_within_sla']) {
                $slaMet++;
            }
        }
        ...
        $dashboardData = [];
        if ($tab === 'dashboard') {
            // Logins trend (last $period days) for this specific company
            $stmt = $pdo->prepare("
                SELECT DATE(al.created_at) as day, COUNT(*) as total 
                FROM audit_logs al
                JOIN client_profiles cp ON cp.user_id = al.user_id
                WHERE al.action = 'login' AND al.created_at >= ? AND cp.company_id = ?
                GROUP BY DATE(al.created_at) 
                ORDER BY day ASC
            ");
            $stmt->execute([date('Y-m-d H:i:s', time() - ($period * 86400)), $company_id]);
            $dashboardData['logins'] = $stmt->fetchAll();

            // Ticket creation trend for this company
            $stmt = $pdo->prepare("
                SELECT DATE(created_at) as day, COUNT(*) as total 
                FROM tickets 
                WHERE created_at >= ? AND client_user_id IN (SELECT user_id FROM client_profiles WHERE company_id = ?)
            ...
            // Real Zabbix Alerts Data
            if (!empty($hosts)) {
                $hostIds = array_column($hosts, 'hostid');
                $dashboardData['zabbix_active_alerts'] = zbx_get_active_alerts($zbxConfig, $auth, $hostIds);
                $dashboardData['zabbix_alerts_count'] = count($dashboardData['zabbix_active_alerts']);
                $dashboardData['zabbix_alerts_trend'] = zbx_get_alerts_trend($zbxConfig, $auth, $hostIds);
            }
        }
        ...
        if ($tab !== 'sla' && !empty($hosts)) {
            ...
            // 4.1 Batch Get Items for all hosts (chunked to avoid 500 errors)
            $allItems = [];
...
render_header('Cliente · Relatórios', current_user());
?>
<div class="card">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
        <div>
            <h2 style="margin:0">Relatórios</h2>
            <div class="muted">Acesse relatórios técnicos e gerenciais do seu ambiente.</div>
        </div>
    </div>
    ...
    <div class="tabs-container" style="margin-bottom:20px">
        <div class="tabs" style="display:flex; gap:10px; border-bottom: 1px solid var(--border); padding-bottom:10px">
            <a href="?tab=technical&period=<?= $period ?>&host_id=<?= $host_id ?>" class="tab-btn <?= $tab === 'technical' ? 'active' : '' ?>" style="text-decoration:none; padding:8px 16px; border-radius:4px; <?= $tab === 'technical' ? 'background:var(--primary); color:white' : 'color:var(--text)' ?>" onclick="updateHostSelect('technical')">Técnico (Consumo)</a>
            <a href="?tab=sla&period=<?= $period ?>" class="tab-btn <?= $tab === 'sla' ? 'active' : '' ?>" style="text-decoration:none; padding:8px 16px; border-radius:4px; <?= $tab === 'sla' ? 'background:var(--primary); color:white' : 'color:var(--text)' ?>" onclick="updateHostSelect('sla')">SLA & Chamados</a>
            <a href="?tab=dashboard&period=<?= $period ?>" class="tab-btn <?= $tab === 'dashboard' ? 'active' : '' ?>" style="text-decoration:none; padding:8px 16px; border-radius:4px; <?= $tab === 'dashboard' ? 'background:var(--primary); color:white' : 'color:var(--text)' ?>" onclick="updateHostSelect('dashboard')">Dashboard Executivo</a>
            <a href="?tab=billing&period=<?= $period ?>" class="tab-btn <?= $tab === 'billing' ? 'active' : '' ?>" style="text-decoration:none; padding:8px 16px; border-radius:4px; <?= $tab === 'billing' ? 'background:var(--primary); color:white' : 'color:var(--text)' ?>" onclick="updateHostSelect('billing')">Billing & Custos</a>
            <a href="?tab=kpi&period=<?= $period ?>" class="tab-btn <?= $tab === 'kpi' ? 'active' : '' ?>" style="text-decoration:none; padding:8px 16px; border-radius:4px; <?= $tab === 'kpi' ? 'background:var(--primary); color:white' : 'color:var(--text)' ?>" onclick="updateHostSelect('kpi')">KPIs Globais</a>
            <a href="?tab=gerencial&period=<?= $period ?>" class="tab-btn <?= $tab === 'gerencial' ? 'active' : '' ?>" style="text-decoration:none; padding:8px 16px; border-radius:4px; <?= $tab === 'gerencial' ? 'background:var(--primary); color:white' : 'color:var(--text)' ?>" onclick="updateHostSelect('gerencial')">Resumo Gerencial</a>
        </div>
    </div>
    ...
    function updateHostSelect(tab) {
        ...
        if (tab === 'technical') {
            ...
            if (select.value === "") {
                // If "All" was selected, try to select the first available host
            ...
            <div id="report-container">
                <?php foreach ($reportData as $idx => $data): ?>
                    <?php 
                        ...
                        // CPU Current & vCPUs
                        $currentCpu = $getLastVal($items['cpu'] ?? null);
                        if ($currentCpu === null) {
                            $idle = $getLastVal($items['cpu_idle'] ?? null);
                            if ($idle !== null) $currentCpu = 100 - $idle;
                            else {
                                $currentCpu = $getLastVal($items['cpu_load'] ?? null);
                            }
                        }
                        $cpuCount = (int)($data['cpu_count'] ?? 0);
                        if ($cpuCount <= 0 && isset($items['cpu_count']['lastvalue'])) {
                            $cpuCount = (int)$items['cpu_count']['lastvalue'];
                        }
                        ...
                        // Disks handled in the loop below
                        $hasDisks = !empty($items['disks']);
                    ...
                    <div class="host-report-section" style="margin-bottom: 60px;">
                        ...
                        <div class="report-dash-grid">
                            ...
                            <div class="report-resource-card">
                                <div class="report-resource-header">
                                    ...
                                    <div class="resource-badge"><?= $cpuCount ?: 'N/A' ?> vCPUs</div>
                                </div>
                                <div class="report-resource-value"><?= $currentCpu !== null ? round($currentCpu, 1) . '%' : 'N/A' ?></div>
                            ...
                            <div class="report-resource-card">
                                ...
                                <div style="display: flex; gap: 20px; margin-top: 4px;">
                                    <div>
                                        <div class="resource-subtext" style="margin-bottom: 4px;">Download</div>
                                        <div style="font-weight: 800; color: var(--primary); font-size: 18px;"><?= format_rate($netInRate) ?></div>
                                    </div>
                                    <div>
                                        <div class="resource-subtext" style="margin-bottom: 4px;">Upload</div>
                                        <div style="font-weight: 800; color: var(--primary); font-size: 18px;"><?= format_rate($netOutRate) ?></div>
                                    </div>
                                </div>
                                <div class="report-resource-chart">
                        ...
                        (function() {
                            ...
                            <?php 
                                ...
                                $checkKeys = ['cpu', 'cpu_idle', 'mem_used', 'mem_pused', 'net_in', 'net_out'];
    ...
    <?php elseif ($tab === 'sla'): ?>
                ...
                <div class="kpi-grid" style="display:grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap:20px; margin-top:20px">
                    <?php
                        ...
                        foreach ($ticketData as $tk) {
                            $totalSli += (float)($tk['sli_seconds'] ?? 0);
                            $totalSlo += (float)($tk['slo_seconds'] ?? 0);
                            if (($tk['is_within_sla'] ?? false)) $withinSla++;
                            $countSli++;
                        }
                    ...
                    <div class="dash-card" style="padding:15px; text-align:center">
                        ...
                        <div style="font-size:2em; font-weight:800; color:var(--primary)">
                            <?php
                                $h = (int)floor($avgSli / 3600);
                                $m = (int)floor((fmod($avgSli, 3600)) / 60);
        ...
        <?php else: ?>
            <div class="report-content-card" style="background:var(--bg); border:1px solid var(--border); border-radius:8px; padding:40px; text-align:center">
                ...
                <div class="muted">Clique em "Gerar Relatório" para visualizar o desempenho de atendimento</div>
            </div>
        <?php endif; ?>
    ...
    <?php elseif ($tab === 'dashboard'): ?>
        <?php if ($generate && !empty($reportData)): ?>
            <div class="dashboard-executivo" style="background:var(--bg); border:1px solid var(--border); padding:30px; border-radius:12px; margin-top:20px; color:var(--text); font-family:'Segoe UI', Tahoma, Geneva, Verdana, sans-serif">
                ...
                <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap:20px; margin-bottom:30px">
                    <?php
                        // Calculate availability for dashboard
                        $totalAvailability = 0; $countAvailability = 0;
                        foreach ($reportData as $data) {
                            $items = $data['items'];
                            $trends = $data['trends'];
                            if ($items['availability'] && isset($data['trends'])) {
                                $avg = 0; $c = 0;
                                foreach ($trends as $t) {
                                    if ($t['itemid'] == $items['availability']['itemid']) {
                                        $avg += $t['value_avg']; $c++;
                                    }
                                }
                                if ($c > 0) {
                                    $totalAvailability += ($avg / $c);
                                    $countAvailability++;
                                }
                            }
                        }
                        $availabilityVal = $countAvailability > 0 ? round(($totalAvailability / $countAvailability) * 100, 1) : 100;

                        $kpis = [
                            // ['title' => 'Logins', 'val' => array_sum(array_column($dashboardData['logins'] ?? [], 'total')), 'color' => '#ff9800', 'data' => array_column($dashboardData['logins'] ?? [], 'total')],
                    ...
                    <script>
                    (function() {
                        const data = <?= json_encode($kpi['data']) ?>;
                ...
                <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(400px, 1fr)); gap:20px">
                    ...
                    <h4 style="margin:0 0 20px 0; color:var(--text)">Disponibilidade dos Ativos (Top 10)</h4>
                    ...
                    <div style="background:var(--panel); border:1px solid var(--border); padding:20px; border-radius:8px; grid-column: span 2">
                    <h4 style="margin:0 0 20px 0; color:var(--text)">Uso de Recursos Global</h4>
                        <div style="height:300px">
                    ...
                    <div style="background:var(--panel); border:1px solid var(--border); padding:20px; border-radius:8px">
                    <h4 style="margin:0 0 20px 0; color:var(--text)">SLA por Categoria</h4>
                        <div style="height:300px">
                            <canvas id="slaChartDashboard"></canvas>
                        </div>
                    </div>
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    // Global resource calculation
                    const days = <?= json_encode(array_map(function($d){ return date('d/m', strtotime($d['day'])); }, $dashboardData['tickets_trend'] ?? [])) ?>;
                ...
                new Chart(document.getElementById('availabilityChartDashboard'), {
                    ...
                    new Chart(document.getElementById('slaChartDashboard'), {
                        type: 'doughnut',
                        data: {
                ...
                function generatePDF() {
                    ...
                    const company = "<?= h($company_name ?? 'Relatorio') ?>";
        ...
        <?php else: ?>
            <div class="report-content-card" style="background:var(--bg); border:1px solid var(--border); border-radius:8px; padding:40px; text-align:center">
                ...
                <div class="muted">Visualize seus indicadores estratégicos em tempo real.</div>
            </div>
        <?php endif; ?>
    ...
    <?php elseif ($tab === 'billing'): ?>
        <?php
            $boletos = [];
            if ($company_id) {
                // Get all users for this company to find all related boletos
                ...
                if (!empty($uids)) {
                    ...
                    $params = array_merge($uids, [date('Y-m-d H:i:s', time() - ($period * 86400))]);
                    $stmt->execute($params);
        ...
        <div class="dash-card" style="margin-top:20px; padding:20px">
            ...
            <div class="muted">Visão de seus custos e faturamento no período de <?= $period ?> dias.</div>
            
            <?php if (!empty($boletos) || ($generate && !empty($hosts))): ?>
                <div class="kpi-grid" style="display:grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap:20px; margin-top:20px">
                    <div class="dash-card" style="padding:15px; text-align:center">
            ...
            <?php else: ?>
                <div class="muted" style="text-align:center; padding:40px">Clique em "Gerar Relatório" para visualizar os custos detalhados por host.</div>
            <?php endif; ?>
        </div>
            ...
            <div class="dash-card" style="margin-top:20px; padding:20px">
                <div style="display:flex; justify-content:space-between; align-items:flex-start">
                    <div>
                        ...
                        <div class="muted">Visão geral consolidada de sua infraestrutura e atendimento no período de <?= $period ?> dias.</div>
                    </div>
                    <div>
                         <button onclick="generatePDF()" class="btn primary">Imprimir / Salvar PDF</button>
                    </div>
                    ...
                    <tbody>
                        <?php foreach ($reportData as $data): ?>
                            <?php
                                ...
                                $totalD = 0; $usedD = 0;
                                if (!empty($items['disks'])) {
                                    foreach ($items['disks'] as $mount => $d) {
                                        if ($d['total'] && isset($grouped[$d['total']['itemid']])) {
                                            $totalD += end($grouped[$d['total']['itemid']])['value_avg'];
                                        }
                                        if ($d['used'] && isset($grouped[$d['used']['itemid']])) {
                                            $usedD += end($grouped[$d['used']['itemid']])['value_avg'];
                                        }
                                    }
                                    if ($totalD > 0) {
                                        $diskTotalStr = format_bytes($usedD) . ' / ' . format_bytes($totalD);
                                    } elseif ($usedD > 0) {
                                        $diskTotalStr = format_bytes($usedD);
                                    }
                                }
    ...
    <?php elseif ($tab === 'kpi'): ?>
        <?php if ($generate && !empty($reportData)): ?>
            ...
            <div class="kpi-grid" style="display:grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap:20px; margin-top:20px">
                <?php
                    ...
                    foreach ($reportData as $data) {
                        ...
                        // Disk
                        if (isset($items['disk_used']) && $items['disk_used'] && isset($grouped[$items['disk_used']['itemid']])) {
                            $last = end($grouped[$items['disk_used']['itemid']]);
                            $totalDisk += $last['value_avg'];
                        }
                        if (isset($items['disk_total']) && $items['disk_total'] && isset($grouped[$items['disk_total']['itemid']])) {
                    ...
                    foreach ($reportData as $data) {
                        $items = $data['items'];
                        $trends = $data['trends'];
                        if (isset($items['availability']) && $items['availability'] && isset($data['trends'])) {
                            $avg = 0; $c = 0;
                            foreach ($trends as $t) {
                                if ($t['itemid'] == $items['availability']['itemid']) {
                                    $avg += $t['value_avg']; $c++;
            ...
            <div class="dash-card" style="margin-top:20px; padding:20px">
                ...
                <div class="muted">Indicadores gerais de performance do seu ambiente.</div>
                <div style="margin-top:20px; height:10px; background:#eee; border-radius:5px; overflow:hidden">
                    <div style="width:<?= $availabilityVal ?>%; height:100%; background:var(--success)"></div>
                </div>
                <div style="display:flex; justify-content:space-between; margin-top:5px; font-size:0.8em">
                    <span>Disponibilidade Média</span>
                    <span style="font-weight:700; color:var(--success)"><?= $availabilityVal ?>%</span>
                </div>
            </div>
...
@media print {
    ...
    /* Ensure background colors and borders are printed */
    * {
        -webkit-print-color-adjust: exact !important;
        print-color-adjust: exact !important;
    }

    /* Main containers for print */
    ...
    /* Executive Dashboard adjustments */
...
<div id="report-header-print">
    <div style="display:flex; justify-content:space-between; align-items:center">
        ...
        <div style="text-align:right">
            ...
            <div style="font-weight:700">Empresa: <?= h($company_name) ?></div>
        </div>
    </div>
</div>
